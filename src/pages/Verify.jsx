...
const decodeCertificateParam = (param, location) => {
  // Extract certificate number from URL path
  // URL format: https://cv.darecentre.in/verify/DARE/AIR/LP/25-26/001
  let certificatePath = '';

  // Try to get from React Router wildcard param first
  if (param && param !== '*') {
  ...
  try {
    ...
    let decoded = decodeURIComponent(certificatePath);
    
    // Normalize: ensure sequence separator is slash (convert -001 to /001)
    decoded = decoded.replace(/-(\d{3,})$/, '/$1');
    
    return decoded;
  } catch (error) {
    // If decoding fails, try without decodeURIComponent
    try {
      return certificatePath.replace(/-(\d{3,})$/, '/$1');
    } catch {
      return convertLegacyHyphenToSlash(certificatePath);
    }
  }
};
...
const Verify = () => {
  ...
  // Get wildcard match from route (everything after /verify/)
  const certificateNo = params['*'] || '';
  const initialCertificate = decodeCertificateParam(certificateNo, location);
          ...
          <div className="relative">
            ...
            <ul className="mt-4 space-y-3 sm:mt-6 sm:space-y-4">
              ...
              <li className="group flex items-start gap-3 rounded-xl bg-white/60 p-3 transition-all duration-200 hover:bg-white/80 hover:shadow-md sm:gap-4 sm:p-4">
                ...
                <span className="pt-1 text-sm leading-relaxed text-slate-700 sm:pt-2 sm:text-base">Use the exact format including slashes, e.g. <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-xs font-semibold text-primary sm:px-2 sm:text-sm">DARE/AIR/LP/25-26/001</code>.</span>
              </li>
            </ul>
          </div>
            ...
            <div className="relative">
              ...
              <ul className="mt-4 space-y-2 sm:mt-6 sm:space-y-3">
                <li className="flex items-center gap-2 rounded-xl bg-white/80 p-2.5 transition-all duration-200 hover:bg-white hover:shadow-md sm:gap-3 sm:p-3">
                  ...
                  <a href="tel:+917812876787" className="font-display text-sm font-semibold text-primary transition-colors hover:text-primary/80 sm:text-base break-all">
                    +91 7812876787
                  </a>
                </li>
                <li className="flex items-center gap-2 rounded-xl bg-white/80 p-2.5 transition-all duration-200 hover:bg-white hover:shadow-md sm:gap-3 sm:p-3">
              ...
              <div className="mt-6 rounded-xl bg-gradient-to-br from-white to-slate-50/50 p-4 text-center shadow-lg ring-1 ring-slate-200/50 sm:mt-8 sm:rounded-2xl sm:p-5">
                ...
                <p className="mt-2 font-mono text-base font-bold text-secondary break-all sm:mt-3 sm:text-lg md:text-xl">DARE/AIR/LP/25-26/001</p>
              </div>
            </div>
          </div>
        </section>
